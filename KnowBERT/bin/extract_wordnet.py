...
def extract_wordnet_from_custom_xml(input_xml_file, entity_output_file, relation_output_file):
    tree = ET.parse(input_xml_file)
    root = tree.getroot()

    with open(entity_output_file, 'w') as fent, open(relation_output_file, 'w') as frel:
        for synset in root.findall('Synset'):
            
            pos = synset.get('pos')

            literal = synset.find('Literal')
            lemma = literal.text.replace(" ","_")
            synset_id = literal.text.replace(" ","_")
            node = {
                'id': synset_id,
                'pos': pos,
                'lemmas': [lemma],
            ...
            for relation in synset.findall('Relation'):
                target_id = relation.get('target').replace(" ","_")
                relation_type = relation.get('type')
                frel.write('{}\t{}\t{}\n'.format(synset_id, 'synset_' + relation_type, target_id))
...
if __name__ == '__main__':
    ...
    if args.extract_graph:
        ...
        extract_wordnet_from_custom_xml("kg_mine/custom_kg.xml", args.entity_file, args.relationship_file)
