        ...
        if i == len(document) - 1 or current_length >= word_target_seq_length:
            if current_chunk:
                ...
                if len(current_chunk) == 1 or np.random.random() < 0.5:
                    ...
                    if len(random_document) > 0:  # Add this condition
...
def read_file_sample_nsp_write_shards(file_glob, output_prefix, num_output_files):
    ...
    for tokens_a, tokens_b, label in read_files(file_glob):
        ...
        if (len(tokens_a) >= 10) and (len(tokens_b)>=10):
...
def main():
    '''
    parser = argparse.ArgumentParser(description="Group sentences by length, do the NSP sampling, and write out files for training.")
    parser.add_argument("--input_glob", type=str, required=True, help="Glob pattern for input text files.")
    parser.add_argument("--output_prefix", type=str, required=True, help="Prefix for output files.")
    parser.add_argument("--num_output_files", type=int, default=10, help="Number of output files to create (default: 10).")
    '''

    #args = parser.parse_args()

    read_file_sample_nsp_write_shards("output_corpus/*.txt", "processed_corpus/", 10)
