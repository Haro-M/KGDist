...
def extract_tucker_embeddings_(tucker_archive, vocab_file, tucker_hdf5):
...
def extract_tucker_embeddings(tucker_archive, vocab_file, tucker_hdf5):
    ...
    # get embeddings
    tmp = torch.load("/home/lpz/mhl/KG_Distill/kb/wordnet_tucker/best.th")

    
    
    embed = tmp['kg_tuple_predictor.entities.weight'].detach().cpu().numpy()
    out_embeddings = np.zeros((NUM_EMBEDDINGS, embed.shape[1]))
    
    #vocab = tmp.vocab

    for k, entity in enumerate(vocab_list):
        embed_id = k+1 #vocab.get_token_index(entity, 'entity')
        if entity in ('@@MASK@@', '@@NULL@@'):
            # these aren't in the tucker vocab -> random init
            out_embeddings[k + 1, :] = np.random.randn(1, embed.shape[1]) * 0.004
        elif entity != '@@UNKNOWN@@':
