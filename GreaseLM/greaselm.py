...
def train(args, resume, has_test_split, devices, kg):
    ...
    for epoch_id in trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        ...
        model.eval()
        preds_path = os.path.join(args.save_dir, 'obqa'.format(epoch_id))
        ...
        if args.save_model:
            ...
            torch.save(checkpoint, model_path +"obqa_checkpoint.pt")
        model.train()
        start_time = time.time()
...
def evaluate(args, has_test_split, devices, kg):
    ...
    model_checkpoint = torch.load("/home/lpz/MHL/KG_Distillation/GreaseLM/pretrained_model/pytorch_model.bin")
    model.to(devices[1])
    model.lmgnn.concept_emb.to(devices[0])
    model.eval()
...
def distill(args, has_test_split, devices, kg):
    assert args.load_model_path is not None
    load_model_path = args.load_model_path
    print("loading from checkpoint: {}".format(load_model_path))
    checkpoint = torch.load(load_model_path, map_location='cpu')

    train_statements = "/home/lpz/MHL/KG_Distillation/GreaseLM/data/obqa/statement/train.statement.jsonl"
    dev_statements = "/home/lpz/MHL/KG_Distillation/GreaseLM/data/distill_obqa/statement/distill.statement.jsonl"
    test_statements = "/home/lpz/MHL/KG_Distillation/GreaseLM/data/distill_obqa/statement/distill.statement.jsonl"
    train_adj = "/home/lpz/MHL/KG_Distillation/GreaseLM/data/obqa/graph/train.graph.adj.pk"
    dev_adj = "/home/lpz/MHL/KG_Distillation/GreaseLM/data/distill_obqa/graph/distill.graph.adj.pk"
    test_adj = "/home/lpz/MHL/KG_Distillation/GreaseLM/data/distill_obqa/graph/distill.graph.adj.pk"
    debug = args.debug
    inhouse = args.inhouse
    ...
    entities_list = init_entities()
    
    relation_list, nesting_relation_list = init_relations()
    ...
    scatter_entities_degree = 0
    
    focus_entities = entities_list[0:10]
    
    focus_relation = relation_list

    query_batch_size = 4
    
    entity_max_batch_size = 10
    
    unsearched_entities = list(set(entities_list)-set(checked_entities))
    
    nesting_relation_depth_limit = 3
    ...
    for epoch in range(epochs):
        ...
        if chunk_query_flag == 0:
            fid_threshold = 15
            queries_count = make_questions(focus_entities, unsearched_entities, focus_relation, query_batch_size, chunk_query_flag)
            
        elif(chunk_query_flag == 0.5):
            fid_threshold = 15
            queries_count = make_questions(focus_entities, next_epoch_entities, focus_relation, query_batch_size, chunk_query_flag)
        else:
            fid_threshold = 10
            queries_count = make_questions(focus_entities, unsearched_entities, focus_relation_, query_batch_size, chunk_query_flag)
            
        with open("recovery_data.txt","w") as f:
           f.write(str(focus_entities))
           f.write('\n')
           f.write(str(unsearched_entities))
        ...
        print ('args.train_statements', train_statements)
        print ('args.dev_statements', dev_statements)
        print ('args.test_statements', test_statements)
        print ('args.train_adj', train_adj)
        print ('args.dev_adj', dev_adj)
        print ('args.test_adj', test_adj)
        ...
        with open(test_statements, "r", encoding="utf-8") as fin:
            
            for line in fin:
                input_json = json.loads(line)
                label = ord(input_json.get("answerKey", "A")) - ord("A")
                output.append((input_json['id'],input_json["statements"]))        
        ...
        with torch.no_grad():
            for qids, labels, *input_data in tqdm(dev_dataloader, desc="Dev/Test batch"):
                count += 1
                
                bs = labels.size(0)
                logits, _ = model(*input_data)
                predictions = logits #[bsize, ]
                fidelity = logits
                preds_ranked = (-logits)
                
                for i, (qid, label, pred, fid) in enumerate(zip(qids, labels, predictions, fidelity)):
                        for indx, state in output:
                            if(indx == str(qid)):
                                cnt = 0
                                if(fid[i] > fid_threshold):
                                    cnt = cnt + 1
                                    reconstruct_sentences.append(state[i]['statement'].replace('.',''))
                                    fidelities.append(fid[i])
                                if cnt >= 2:
                                    with open('output.txt', 'a') as f:
                                        for i in range(len(fid)):
                                            f.write(str(state[i]['statement']) + " Fid = " + str(fid[i]) + "\n")
                                        f.write("Correct: "+str(cnt)+" \n\n")
          

        next_epoch_entities = distill_test(reconstruct_sentences, fidelities, False, focus_entities, relation_list, epoch, fid_threshold)
        
        if(chunk_query_flag == 1 and len(next_epoch_entities) > 0):
            chunk_query_flag = 0.5
            continue
        if(chunk_query_flag == 0.5):
            chunk_query_flag = 1
            
        checked_entities = list(set(focus_entities).union(set(checked_entities)))
            
        unsearched_entities = list(set(entities_list)-set(checked_entities))
        
        if(len(next_epoch_entities) > 0):
            scatter_entities_degree = 0
            focus_entities = list(set(next_epoch_entities) - set(checked_entities))
        elif(len(unsearched_entities) == 0):
            kg_prefine(nesting_relation_list, nesting_relation_depth_limit)      
            break
        elif(len(unsearched_entities) >= query_batch_size) :
            scatter_entities_degree = scatter_entities_degree + 1
            focus_entities = unsearched_entities[0: (10 * scatter_entities_degree) % entity_max_batch_size % len(unsearched_entities)]
        else:
            focus_entities = unsearched_entities    
            
        if(scatter_entities_degree >= 1):
            chunk_query_flag = 0
                
        if(len(focus_entities) == 0):
            focus_entities = unsearched_entities[0:10]
            chunk_query_flag = 1
            
        print("Distillation Epoch = ", epoch, " ||| Focusing Entity Count: ",len(focus_entities), " ||| Uncleared Entity Count: ",len(unsearched_entities), " ||| Query Count: ", queries_count)
...
def main(args):
    ...
    with wandb.init(project="KG-LM", config=args, name=args.run_name, resume="allow", id=wandb_id, settings=wandb.Settings(start_method="fork"), mode=wandb_mode):
        ...
        elif "eval_distill" in args.mode:
            eval_distill(args, has_test_split, devices, kg)
        elif "eval" in args.mode:
            evaluate(args, has_test_split, devices, kg)
        elif "distill" in args.mode:
            distill(args, has_test_split, devices, kg)
        else:
...
if __name__ == '__main__':
    ...
    # General
    parser.add_argument('--mode', default='train', choices=['train', 'eval', 'distill'], help='run training or evaluation')
