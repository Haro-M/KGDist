...
def ground_qa_pair(qa_pair):
    ...
    if len(question_concepts) == 0:
        question_concepts = hard_ground(nlp, s, CPNET_VOCAB)
          # not very possible
    ...
    #print({"sent": s, "ans": a, "qc": question_concepts, "ac": answer_concepts})
...
def ground_mentioned_concepts(nlp, matcher, s, ans=None):
    ...
    if(' is a ' in s):
        ...
        verb = ' belongs to '
        verb_list = ['belongs', 'belonged', 'belong']
    elif('effects' in s):
        verb = ' affects '
        verb_list = ['affects', 'affected', 'affect']
    elif('causes' in s):
        verb = ' causes '
        verb_list = ['causes', 'caused', 'cause']
    elif('related' in s):
        verb = ' is related to '
        verb_list = ['relates', 'relate', 'related']
    elif('is used for' in s):
        verb = ' is used for '
        verb_list = ['used', 'uses', 'use']
        
    if(len(verb_list) > 0):
        stA = s.split(verb)[0]
        if(len(s.split(verb)) > 1):
          stB = s.split(verb)[1]
          noun_list.append(stB.replace(' ','_'))
        noun_list.append(stA.replace(' ','_'))
    
    if(ans!=None):
        noun_list_ans.append(ans.replace(' ','_'))
    
    mentioned_concepts = set(verb_list).union(set(noun_list)) - set(noun_list_ans)
